...
namespace org.GraphDefined.Vanaheimr.Hermod.MCP
{
    ...
    /// <see cref="SendMessageAsync(AJSONRPCMessage, CancellationToken)"/> and <see cref="DisposeAsync()"/> methods
    ...
    public abstract partial class TransportBase : ITransport
    {
        ...
        private readonly Channel<AJSONRPCMessage> _messageChannel;
        ...
        /// </summary>
        internal TransportBase(String                     name,
                               Channel<AJSONRPCMessage>?  messageChannel,
                               ILoggerFactory?            loggerFactory)
        {
            ...
            _messageChannel  = messageChannel
                                   ?? Channel.CreateUnbounded<AJSONRPCMessage>(
        ...
        /// <inheritdoc/>
        public ChannelReader<AJSONRPCMessage> MessageReader => _messageChannel.Reader;

        /// <inheritdoc/>
        public abstract Task SendMessageAsync(AJSONRPCMessage message, CancellationToken cancellationToken = default);
        ...
        protected async Task WriteMessageAsync(AJSONRPCMessage message, CancellationToken cancellationToken = default)
        {
            if (!IsConnected)
            {
            ...
            {
                var messageId = (message as AJSONRPCMessageWithId)?.Id.ToString() ?? "(no id)";
